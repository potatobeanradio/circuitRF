...
///
/// Per-stage / composite powers from the INl convention (HbEngine "INl current-direction"):
///   Pin  into a stage  = +½·Re(V_gate  · conj(INl_gate))
///   Pout of a stage    = −½·Re(V_drain · conj(INl_drain))   (drain delivers into its external net)
/// </summary>
public class Hero4Tests(ITestOutputHelper output)
{
    ...
    public void Hero4_TwoStage_DriveUp_CompositeGainCompresses()
    {
        ...
        int g1 = NodeIdx(result, "n_gate");
        int d1 = NodeIdx(result, "n_drain");
        int g2 = NodeIdx(result, "n_gate2");
        int d2 = NodeIdx(result, "n_drain2");
        ...
        foreach (var s in result.Trace.Steps)
            Assert.True(s.Converged,
                $"Hero4 did not converge at Pavl={s.Pin_dBm} dBm (‖F‖={s.IterTrace.LastOrDefault()?.ResidualNorm:E2}).");
        ...
        for (int i = 0; i < n; i++)
        {
            double pavlDbm = result.SweepValues[i];
            double pavlW   = Math.Pow(10, (pavlDbm - 30) / 10);

            double pin1  = PinW (result.V[i], result.INl[i], g1);
            double pout1 = PoutW(result.V[i], result.INl[i], d1);
            double pin2  = PinW (result.V[i], result.INl[i], g2);
            double pout2 = PoutW(result.V[i], result.INl[i], d2);
        ...
        double poutSatDbm = 10 * Math.Log10(PoutW(result.V[n - 1], result.INl[n - 1], d2)) + 30;
    ...
    private static int NodeIdx(HbResult r, string name)
        => Array.FindIndex(r.InterfaceNodeNames, s => s.Equals(name, StringComparison.OrdinalIgnoreCase));

    // Pin delivered into a stage at its gate: +½ Re(V · conj(INl)).
    private static double PinW(Complex[,] V, Complex[,] I, int gate)
        => 0.5 * (V[gate, 1] * Complex.Conjugate(I[gate, 1])).Real;

    // Pout delivered by a stage at its drain into the external network: −½ Re(V · conj(INl)).
    private static double PoutW(Complex[,] V, Complex[,] I, int drain)
        => -0.5 * (V[drain, 1] * Complex.Conjugate(I[drain, 1])).Real;
}
