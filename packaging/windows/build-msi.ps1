<#
  ...
  Two install scopes, and the difference is which one can update itself:

    -Scope perMachine  (default)  %ProgramFiles%\circuitRF        NOTIFY-ONLY
                                  dist\circuitRF-<VERSION>-<Arch>.msi
    -Scope perUser                %LOCALAPPDATA%\Programs\circuitRF   updates silently
                                  dist\circuitRF-<VERSION>-win-<Arch>-user.msi
                                  dist\circuitRF-<VERSION>-win-<Arch>.zip   <- the update payload

  A silent background update requires an install location the running user can write, which
  %ProgramFiles% is not - see the long note at the top of circuitRF.wxs. The perUser build also
  emits the .zip the updater fetches; its name is parsed by src\Ui\Updates\UpdateAssetNames.cs and
  is asserted by tests\Ui.Tests\PackagingScriptTests.cs, because renaming it would stop updates
  with no error anywhere and no user report.
...
param(
    ...
    # perMachine is what circuitRF has always shipped and it stays the default. perUser is the
    # channel that can update itself; see the header above.
...
# THE ARTIFACT NAMES. The .zip spelling is a contract with the updater, not a preference - see
# src\Ui\Updates\UpdateAssetNames.cs and PackagingScriptTests.cs.
if ($perUser) {
