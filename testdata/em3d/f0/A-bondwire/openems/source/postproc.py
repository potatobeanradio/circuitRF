...
with open(out, 'w') as fh:
    ...
    fh.write('! S22 = S11 and S12 = S21 by the mirror symmetry of the geometry (not a second run)\n')
...
for k in [0, 9, 19, 39]:
    print(f'{f[k]/1e9:4.0f} GHz |S11| {20*np.log10(abs(S11[k])):8.3f} dB  |S21| {20*np.log10(abs(S21[k])):8.4f} dB  ang S21 {np.degrees(np.angle(S21[k])):8.3f}')
# cross-check against openEMS's own CalcPort (upstream post-processing)
try:
    from openEMS.ports import LumpedPort
    import CSXCAD
    CSX = CSXCAD.ContinuousStructure()
    p1 = LumpedPort(CSX, 1, 50, [-550, -50, 0], [-550, 50, 100], 'z', excite=1.0)
    p2 = LumpedPort(CSX, 2, 50, [550, -50, 0], [550, 50, 100], 'z')
