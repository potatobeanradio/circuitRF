...
case $(uname -m) in
    ...
    x86_64)        arch=x86_64;  url=$ALPINE_NETBOOT_X86_64_URL;  want=$ALPINE_NETBOOT_X86_64_SHA256 ;;
...
# ── Kernel ────────────────────────────────────────────────────────────────────
# Stock arm64 kernels are EFI *zboot* images: a PE wrapper whose payload is a compressed kernel,
# meant to be unpacked by its own EFI stub. VZLinuxBootLoader does not run an EFI stub and needs a
# raw uncompressed Image, so the payload is extracted here. Getting this wrong is not subtle but it
# is opaque: the VM refuses to start with nothing but "Internal Virtualization error".
echo "==> extracting kernel"
python3 - "$work/nb/boot/vmlinuz-virt" "$out/crf-linux-kernel" <<'PY'
...
# An arm64 Image carries "ARM\x64" at offset 56. Checking it here turns a wrong-format kernel into a
# clear message now, instead of an unexplained VM start failure later.
if len(image) > 60 and image[56:60] not in (b'ARM\x64', b'ARMd'):
    if image[:4] not in (b'\x7fELF', b'MZ\x00\x00'):
        print("    warning: no arm64 Image magic found; the kernel may not boot", file=sys.stderr)
...
# ── x86-64 glibc runtime (Apple Silicon only) ────────────────────────────────
