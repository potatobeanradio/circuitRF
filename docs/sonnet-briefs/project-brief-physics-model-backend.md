...
| Op-vars (operating-point outputs) | **Built (2026-09-03), by PM3 — `project-brief-physics-model-opvar-readback.md`.** No longer an omission: `describe` declares them, an `OP` cube carries them at every DC and HB point, and `DC1.OP("X1.gm")` names one. They remain excluded from the settable list, which is the property that makes this read-*back*. | `emit_describe`, `cmd_opvars`, `DcResultPacker`, `HbOpVars` |
