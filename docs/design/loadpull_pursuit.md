...
### 5.0 The written `.gam` format (`GamWriter`)
The builder emits a file `GamReader` reads back exactly (loadpull.md §2.2). It is **always written in impedance form** with the complex-literal column format — never Γ:
```
# impedance Z0=50 re+j*imag
# Generated by circuitRF GamWriter (loadpull_pursuit.md §5)
# WARNING: Removed 1 grid point(s) within 1.05 VSWR of non-convergent terminations (keepNonconvergingPoints=false).
33.33333333+20.83333333j
47.22222222-6.944444444j
75+0j
…
```
- **Header** — `# impedance Z0=<z0> re+j*imag` plus a provenance comment line; any exclusion warning is recorded as one `# WARNING:` line so the file is self-documenting.
- **Points** — one `re+j*imag` impedance per line, real and imaginary parts at up to 10 significant figures, with an explicit `+`/`-` before the imaginary part and a trailing `j` (e.g. `75+0j`, `47.22222222-6.944444444j`).

**Frequency-swept pursuit → multi-frequency `.gam`.** When a pursuit is wrapped in a tone-frequency sweep, the writer accumulates **one frequency-tagged block per frequency** into a single file (`WriteFile(..., freqHz: f, append: true)`): the `#` header is written only for the new/empty file, then each frequency contributes a bare `freq=<f>Hz` delimiter line followed by that frequency's recommended terminations. The result is exactly the multi-frequency layout `GamReader` selects from per analysis frequency (loadpull.md §2.2.1), so the recommended grid feeds straight back into a frequency-swept follow-on loadpull.
