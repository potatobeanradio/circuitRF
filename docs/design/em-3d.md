...
>
> **rev 3 → rev 4 (2026-09-24):** **no new materials file** — the technology (`.ctech`) gains named
> materials, and a stackup layer that names one takes that material's values (§4.1a). **Bond wires in
> 3D** get a flat-bottomed **hexagonal** cross-section by default, perimeter-matched to the round wire;
> a **bond style per end** (wedge by default, or ball); a **wedge foot** that exists only in the 3D
...
- **Named solids**, each a construction object (extruded polygon, box, cylinder, sphere, swept wire,
  boolean result — §6.3), carrying a **material** by name from the technology (§4.1a);
...
carrying a thermal property. The technology gains:

- **`Materials`** — a list of **named** materials: εr, tanδ, μr, σ at 20 °C and its temperature
  coefficient (the pair `WireMaterial` already carries, and the 85 °C evaluation it already does),
  and for §9 thermal conductivity k (optionally k(T)), density and specific heat. Optionally a
  **direction-dependent εr** (xx/yy/zz) for laminates and crystalline substrates, which Palace accepts
  as a tensor. The shipped bond-wire metals — gold, aluminium, copper, silver — become entries in the
  shipped technologies rather than constants in code.
- **`StackupLayer.Material`** — a stackup entry may **name** one. **Decided: an entry that names a
  material uses that material's values**, for every solver, and its own four numbers are not read. An
  entry that names none keeps its own numbers, so every technology written before this reads exactly
  as it did. The editor keeps the entry's own numbers equal to the named material's (so an older build
  reading the file sees the same values), and `check` warns when a hand edit makes them disagree — an
  edited number that nothing reads must not pass silently.
- **`Bodies`** — 3D-only solids the stackup has no row for: mould compound, a lid, a die attach, a
  solder layer. Each names a material, the stackup entry it **sits on** and a thickness, and either
  the drawing layers that give its outline or none, meaning it covers the whole problem laterally (an
  overmold). A separate list rather than a new `StackupKind`, so an older build simply ignores it and
  the planar extractor, which must never see one, has nothing to skip.
- **Lateral extent.** The planar solvers treat every dielectric as laterally infinite; a 3D problem
  cannot. A dielectric entry extends to the board-outline drawing layer when the layout draws one and
  to the air box otherwise. `PresentWithLayer` — "this film exists only where this mask is drawn" —
  was an approximation the planar extractor made as well as it could; in 3D it is simply true, and the
  film is extruded from the mask.

**Wire metals resolve through the technology in a 3D setup.** A wire's `Material` is looked up in the
technology first. A name only the `.wBond` defines comes from the `.wBond`, which stays
self-contained for kernel W. A name both define differently is reported, and the technology's values
are used, for the same reason a named stackup material wins. Air is implicit: whatever no solid
claims inside the air box.

The fields are additive and nullable, following the pattern of every field the technology has
...
### 4.6 Authoring it headlessly — the same documents, the same verbs

**The 3D problem of Tier A is not a new document to author.** It is generated from a layout
(`.clay`), a technology (`.ctech`) and wirebond designs (`.wBond`), and the setup that runs it is a
`.cem` — the planar setup, with a solver choice and one section per solver (§4.2). So a client (a
script, an agent on the MCP server) authors a 3D run exactly as it authors a planar one: **it writes
those files**, then runs `check`, `explain` and `em`. circuitRF's rule for every capability stands —
the format is the contract, and there are no per-primitive edit verbs (no `add-box`, no `set-port`),
because a verb that edits a document is a second writer that drifts from the first.

**What that needs already exists for 2D** (2026-09-24). `circuitrf reference` serves a generated
page for each format — every field, its JSON type, its default, and a whole example that was run end
to end when written — and the MCP server serves the same pages as resources:

| Topic | Format | Example on the page |
|---|---|---|
| `reference layout` | `.clay` | a 50 Ω microstrip through-line with an EM port at each end |
| `reference em-setup` | `.cem` | that line solved 1–10 GHz, ~5 s |
| `reference wbond` | `.wBond` | a two-wire gold array, and the `.cnl` that simulates it |
| `reference technology` | `.ctech` | (existing) |

Each 3D addition therefore lands on a page with no page to write: §4.1a's material fields on
`technology`, §6.6's wire fields on `wbond`, and the 3D setup's solver sections and ports on
`em-setup`. **What is genuinely new for 3D**, and belongs to F1/F1b:

- **3D ports.** A layout pin is a planar port. 3D adds lumped ports between two named objects and wave
  ports as sheets on the air box, declared in the `.cem` and referring to pins or named objects —
  never to a face index (§6.4).
- **A picture without a GPU.** A client cannot check a 3D model it cannot see. `render` on a 3D setup
  draws section views (XY at a stated z, XZ/YZ cuts) and an isometric outline as SVG, through the same
  Skia renderers every other `render` uses — not the GPU viewer of §8, which needs a window.
  Deterministic output, so a test can compare it byte for byte.
- **The size before the run.** `explain` on a 3D setup reports the resolved solids and their
  materials, the ports, and the mesh or grid size and time step each backend would use, so the
  refusal "this will not fit" or "this will take an hour" comes before the hour, not after it.
...
- **A run is a CLI verb before it is a button** (the rule `docs/design/cli.md` states for every run
  ...
  the command line — or a sibling verb, decided when F1 is briefed. §4.6 leans towards `em`: the 3D
  setup is a `.cem`, and a second verb for the same document would split one format's runs in two.
...
as hexagonal (or round) sweeps with their feet and balls (§6.6), bumps as spheres or truncated spheres,
mould compound and lids from the technology's bodies (§4.1a).
...
### 6.6 Bond wires in 3D — cross-section, feet and loop height

Kernel W treats a wire as a thin filament with a radius; its cross-section never appears. A 3D model
has to state one, and the shape of the wire where it lands on a pad decides whether the model can be
meshed at all.

**The cross-section: a flat-bottomed hexagon by default.** A regular hexagon with a flat top and a
flat bottom, swept along the wire with "up" held fixed, so the bottom face stays parallel to the pad
along the whole wire. That flat face is the point: a round wire resting on a flat pad touches it along
a **line**, and a line contact is the zero-thickness sliver that breaks a tetrahedral mesher (§12's
mesh-robustness risk, in its worst form). A flat face lands **face to face**, the pad and the foot
share a surface after the fragment operation, and the contact has a real area. **Round** stays
available per wire.

**Sizing — decided: perimeter-matched.** Above a few hundred megahertz the current in a bond wire is
confined to a skin a few microns deep (δ ≈ 2.7 µm in gold at 1 GHz and 85 °C, against a 12.7 µm
radius), so resistance is set by the **perimeter** the current flows around, not by the area. The
hexagon is therefore sized so its perimeter equals the round wire's, πd:
...
The cost is stated rather than hidden: **the cross-section area is 9.3 % smaller**, so DC resistance —
and the low-frequency end of the transition, below about 200 MHz for a 1 mil gold wire, where the
current still fills the section — reads about 10 % high, and a thermal solve (§9) sees 9.3 % less
conduction along the wire. A polygon also crowds skin-regime current into its corners, which raises
its resistance somewhat above the equal-perimeter circle's; F0 measures that against a round wire of
the same length rather than assuming it small. The height, flat to flat, is not the diameter — which
is why the loop height below is defined on the wire's actual top and bottom surfaces, never on d.

**Bond style — per END: wedge (the default) or ball.** A wedge–wedge wire is the default and the
dominant case; a ball–wedge wire has a ball at its first bond and a wedge at its second.

- **A wedge end has a foot**: a horizontal run of the wire lying flat on the pad, its bottom face on
  the pad's top surface. **The foot exists only in the 3D model.** It extends from the wire's end point
...
**Loop height — decided: the assembly definition.** In a 3D model, **loop height is measured from the
bottom of the foot — the top surface of the pad or lead it is bonded to — to the TOP surface of the
wire at its apex.** That is what an assembly team measures optically: the pad or lead height, then
the top of the wire. On feet at different heights it is measured from the lower one, as the bonder is
set up. It is the number a manufacturing specification states, so it is the number the most accurate
wire model accepts and reports.

It differs from wBond's own definition (§3.1a of `wbond.md`: the maximum minus the minimum z of the
wire's polyline, which kernel W reads as the filament axis) by about **one wire height** — the
apex's top surface lies half a height above the axis and the foot's bottom half a height below it. For
wBond's Grover-and-thin-wire approximation that difference is inside the model's own accuracy, and
wBond's definition is left as it is. When a 3D wire comes from a `.wBond`, its polyline is read as the
axis, each foot is placed with its bottom face on its pad, and the loop height the 3D setup reports,
and accepts as an override, is the assembly one.
...
the matched perimeter; the viewer shows which was done (§8.5).
...
- **Material properties** — k, k(T), density and specific heat come from the technology's named
  materials (§4.1a), the same entries the electromagnetic solve reads.
...
**Made (2026-09-24, rev 4):**
- **No new materials file**: the technology gains named materials and 3D-only bodies; a stackup entry
  that names a material uses that material's values (§4.1a).
- **Bond wires in 3D are flat-bottomed hexagons by default, perimeter-matched** to the round wire; round
  remains available per wire (§6.6).
- **Bond style per end, wedge by default**; a wedge foot exists **only in the 3D model**, with a foot
  length parameter; a ball end lands vertically (§6.6).
- **3D loop height is the assembly definition**: bottom of the foot to the top of the wire at its apex.
...
7. The default foot length, ball diameter and ball height (§6.6) — F0, against assembly data.
