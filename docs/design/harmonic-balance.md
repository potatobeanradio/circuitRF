...
- **Power (source) stepping** (Heroes 2, 4). Start small-signal (near-linear, easy), step `Pin` up into ≥ 3 dB compression, each step seeded by the last converged `V`. Required by the Hero-2/4 "converges at *every* point" bar.
