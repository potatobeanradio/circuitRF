# `src/Core` — archived changelog

This is the append-only development-phase log that used to be the whole of `src/Core/CLAUDE.md`
(2,156 lines). It was moved here verbatim on **2026-08-13** — line numbers below are unchanged from
the original file — when `CLAUDE.md` was rewritten down to a lean, current-state reference (~260
lines: architecture, standing invariants, "how to X" guidance, and a compressed list of dated
pitfalls). Read `CLAUDE.md` first; it links back here for anything that needs the full story.

**How to use this file:**
- **Grep it** rather than reading it whole — `grep -n "TOhm" src/Core/HISTORY.md`,
  `grep -n "2026-08-08" src/Core/HISTORY.md`, `grep -n "^A kit" src/Core/HISTORY.md`.
- **Chronology runs newest-first for most of the file, then flips.** Lines 1–1841 and roughly
  1950–2154 are dated brief write-ups, prepended at the top each time (so line 1 is the newest
  entry). Lines 1842–1949 are the durable "Standing instructions for `src/Core`" section that used
  to anchor the bottom of the file before the prepend convention took over — that section's content
  now lives in `CLAUDE.md` itself, kept current rather than archived.
- **Every dated entry title is a decent grep anchor** — they read like
  `"<what changed> (<brief-name-or-date>) — COMPLETE"` or similar, one per finding.

**Maintenance rule, or `CLAUDE.md` regrows.** A completed phase's narrative — measured numbers,
test-pass counts, blow-by-blow "brief-xxx" detail — is appended here, never back into `CLAUDE.md`.
`CLAUDE.md` only changes when a phase adds something true of the code *tomorrow*: a new invariant, a
moved default, a new refusal, a trap that now has a name. If a phase adds nothing durable, it adds
