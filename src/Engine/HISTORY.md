# `src/Engine` — archived changelog

This is the append-only development-phase log that used to be the whole of `src/Engine/CLAUDE.md`
(1,764 lines). It was moved here verbatim on **2026-08-13** — content and order unchanged from the
original file — when `CLAUDE.md` was rewritten down to a lean, current-state reference (~430 lines:
architecture, standing invariants, current sweep/result formats, and a compressed list of dated
pitfalls). Read `CLAUDE.md` first; it links back here for anything that needs the full story.

**How to use this file:**
- **Grep it** rather than reading it whole — `grep -n "Phase 4b"`, `grep -n "2026-06-16"`,
  `grep -n "RunControl"`.
- The dated write-ups below run roughly chronologically (oldest phase content first, with the
  standing "Engine — local conventions" preamble further down, since this file preserves the
  original append order rather than re-sorting it).
- Sections prefixed `Mom/ —` are duplicate summaries of phases whose authoritative detail lives in
  `src/Engine/Mom/CLAUDE.md` and `src/Engine/Mom/HISTORY.md`; prefer those files for the Mom kernel
  itself, this copy is kept only for archaeology of what the Engine-level surface looked like at the
  time.
